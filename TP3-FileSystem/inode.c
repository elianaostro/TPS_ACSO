...
int inode_iget(struct unixfilesystem *fs, int inumber, struct inode *inp) {
    // Verificar si el inumber es válido
    if (inumber < 1 || inumber >= (fs->superblock.s_isize * 16)) {
        return -1;
    }
    
    // Calcular en qué bloque y offset está el inode
...
int inode_indexlookup(struct unixfilesystem *fs, struct inode *inp, int blockNum) {
    if (inp->i_mode & ILARG) {
        if (blockNum < (int)MAX_DIRECT_ENTRIES) {
            ...
            if (diskimg_readsector(fs->dfd, inp->i_addr[which_indirect], indirect_block) != DISKIMG_SECTOR_SIZE) {
                return -1;
            }
            ...
            uint16_t indirect_block[DISKIMG_SECTOR_SIZE / sizeof(uint16_t)];
            if (diskimg_readsector(fs->dfd, double_indirect[index1], indirect_block) != DISKIMG_SECTOR_SIZE) {
                return -1;
            }
        ...
        // Archivo simple (bloques directos)
        if (blockNum < 8) {
